  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
         <h1 className="text-2xl font-bold text-slate-800">Historique des Recherches</h1>
      ...
      {history.length === 0 ? (
        <Card className="text-center py-16">
           ...
           <h3 className="text-lg font-semibold text-slate-600">Aucun historique</h3>
           <p className="text-slate-400 max-w-md mx-auto mt-2">
             Vos recherches passées apparaîtront ici pour vous permettre de les consulter à tout moment.
           </p>
        </Card>
      ) : (
                  ...
                  <div className="md:w-1/3 space-y-3 border-b md:border-b-0 md:border-r border-slate-100 pb-4 md:pb-0 md:pr-6">
                     <div>
                        ...
                        <p className="text-xs text-slate-500 flex items-center gap-1.5 mt-1">
                          <Calendar size={12} />
                          {new Date(item.timestamp).toLocaleDateString('fr-BE', { day: 'numeric', month: 'long', hour: '2-digit', minute:'2-digit' })}
                        </p>
                     </div>
                     <div className="flex flex-wrap gap-2">
                        <Badge variant="info">{item.opportunities.length} opportunités</Badge>
                        <Badge variant="neutral">{item.sources.length} sources</Badge>
                     </div>
                  </div>
                  
                  <div className="md:w-2/3 flex flex-col justify-between">
                    <div>
                        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Synthèse</h4>
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-50 flex gap-4 overflow-hidden">
                        ...
                        {item.opportunities.length > 2 && (
                            <span className="text-xs text-slate-400 self-center">+{item.opportunities.length - 2} autres</span>
                        )}
                    </div>
                  </div>
               </div>
      ...
      {isAdmin && (
        <div className="mt-8 p-4 bg-red-50 border border-red-100 rounded-xl flex items-center gap-3 text-red-800 text-sm">
           <ShieldCheck size={20} />
           <p><strong>Mode Administrateur Actif :</strong> Vous avez accès aux fonctions d'exportation complètes des logs.</p>
        </div>
      )}
    </div>
  );
